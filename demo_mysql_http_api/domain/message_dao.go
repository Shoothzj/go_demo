...
type messageRepoInterface interface {
	Get(int64) (*Message, utils.MessageErr)
	Create(*Message) (*Message, utils.MessageErr)
	Update(*Message) (*Message, utils.MessageErr)
	Delete(int64) utils.MessageErr
	GetAll() ([]Message, utils.MessageErr)
...
func (mr *messageRepo) Get(messageId int64) (*Message, utils.MessageErr) {
	stmt, err := mr.db.Prepare(queryGetMessage)
	if err != nil {
		return nil, utils.NewInternalServerError(fmt.Sprintf("Error when trying to prepare message: %s", err.Error()))
	}
	defer stmt.Close()
...
func (mr *messageRepo) GetAll() ([]Message, utils.MessageErr) {
	stmt, err := mr.db.Prepare(queryGetAllMessages)
	if err != nil {
		return nil, utils.NewInternalServerError(fmt.Sprintf("Error when trying to prepare all messages: %s", err.Error()))
	}
	defer stmt.Close()
...
func (mr *messageRepo) Create(msg *Message) (*Message, utils.MessageErr) {
...
func (mr *messageRepo) Update(msg *Message) (*Message, utils.MessageErr) {
	stmt, err := mr.db.Prepare(queryUpdateMessage)
	if err != nil {
		return nil, utils.NewInternalServerError(fmt.Sprintf("error when trying to prepare user to update: %s", err.Error()))
	}
	defer stmt.Close()
...
func (mr *messageRepo) Delete(msgId int64) utils.MessageErr {
	stmt, err := mr.db.Prepare(queryDeleteMessage)
	if err != nil {
		return utils.NewInternalServerError(fmt.Sprintf("error when trying to delete message: %s", err.Error()))
	}
	defer stmt.Close()
